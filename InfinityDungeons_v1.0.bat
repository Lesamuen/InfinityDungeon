...
:mainmenu
cls
echo You are currently playing Infinity Dungeons Release v1.0
...
echo You sit up and look around you.
echo You are in a dark room. There is only one corridor out.
pause >nul
cls
:main
if %hunger% GTR 0 set /a hunger=%hunger%-5
if %hunger% == 0 set /a health=%health%-10&echo You are starving!
...
goto main
:eat
if %food% GTR 0 cls&echo You eat some delicious bread and steak.&echo Your stomach stops growling.&set /a food=%food%-1&set hunger=100&goto main
if %food% == 0 cls&echo You don't have any food to eat!&goto main
:usehealth
if healthpotions == 0 cls&echo You don't have any health potions to drink!&goto main
if healthpotions GTR 0 cls&echo You drank a health potion!&set health=100&set /a healthpotions=%healthpotions%-1&goto main
:helpdungeon
cls
echo help : Lists available commands
echo:
echo nextroom : Go to the next room
echo:
echo inventory : Shows you your inventory
echo:
echo usepotion : Use one of your health potions
echo:
echo eat : Stop starving
echo:
echo save : Saves the game (You may only have 1 at any time)
echo:
echo load : Loads a save file if you have one
echo:
...
cls
echo Corrupted/missing save file! Please restart your game!
goto main
:move
...
rem ----1.x Changes----
rem Massive overhaul by a now-highschooler!!!
rem --1.0--
rem -Spelling error in main menu
rem -Moved saving/loading/new game to new main menu
rem -Removed unnecessary screens in beginning
rem -Reorganized help functions
rem -Reorganized prompts
rem -Renamed functions
rem -Buffed hunger
rem -Removed suicide
rem -Removed unnecessary 4 corridors
rem -Disabled saving and loading...will reimplement after overhaul is done
rem -Changed all echo: to echo:
rem -Changed all :: to rem

rem --TODO--
rem Rename variables
rem Add maxhealth, scale with lvl up
...
rem Combine item pickups, helps, inventories, etc. into single sections using variables
rem Add ability to save/load in battle
rem Mana
rem Mana Potions
rem Magic weapons do more damage but use mana
rem Ranged weapons hit faster but use ammo
rem Different ammo pickups? (Arrows)
rem Mana regen based on INT?
rem Attack Speed (Dexterity???)
rem Retreat chance to fail
rem Thirst & Water
