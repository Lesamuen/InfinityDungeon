...
:mainmenu
cls
echo You are currently playing Infinity Dungeons Release v1.2
...
set /a health-=20
echo WARNING: You are currently starved!
)
...
set /a health-=20
echo WARNING: You are currently dehydrated!
)
...
if "%nextmove%"=="move" goto move
if "%nextmove%"=="continue" goto move
if "%nextmove%"=="cont" goto move
if "%nextmove%"=="rest" goto rest
if "%nextmove%"=="meditate" goto meditate
if "%nextmove%"=="med" goto meditate
if "%nextmove%"=="inventory" goto inventory
if "%nextmove%"=="inv" goto inventory
...
echo:
echo nextroom/move/continue/cont : Goes to the next room
echo:
echo rest : Regain a bit of health and stamina.
echo:
echo meditate/med : Absorb a bit of mana from the dungeon air.
echo:
echo inventory/inv : Shows you your inventory
echo:
...
rem Mana regen
rem Attack Speed
rem Run needs stamina
rem Thirst & Water
rem Miniboss 5% chance & Boss 1% chance
rem Inventory?! (Do arrays exist in batch?) Maybe weight instead of item slot system
rem CRAFTING?!
...
rem Proficiencies for weapon types

rem Copyright (c) 2020 by Samuel Nguyen